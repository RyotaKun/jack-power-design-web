...
7. Drop the demo chrome: each source file has a fixed "Created by Jay from
   RoboLabs" attribution bar pinned to the bottom of the viewport — remove that
   element so it doesn't ship as overlay UI in my product. (Keep the credit in a
   code comment if you like, just not on-screen.)
